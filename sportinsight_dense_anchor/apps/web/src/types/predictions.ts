...
export const PRODUCT_CLASSES = ['Goal', 'Corner', 'Yellow card', 'Red card']

export type SoccerNetResolution = '224p' | '720p' | 'both'
export type SoccerNetSplit = 'train' | 'valid' | 'test' | 'challenge'

export interface SoccerNetDownloadRequest {
  root: string
  password?: string | null
  split: SoccerNetSplit[]
  resolution: SoccerNetResolution
  game?: string | null
  include_features: boolean
  include_labels: boolean
}

export interface SoccerNetDownloadStatus {
  job_id: string
