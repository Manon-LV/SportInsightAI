...
class Prediction:
    ...
    def to_soccernet_dict(self) -> dict:
        """Format attendu par SoccerNet.Evaluation.ActionSpotting.evaluate."""
        return {
            "half": self.half,
            "position": int(self.timestamp * 1000),  # secondes → millisecondes
            "label": self.label,
            "confidence": round(float(self.score), 6),
        }
